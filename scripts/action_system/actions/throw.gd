class_name ActionThrow extends Action
...
func perform(_delta: float, item_node: ActionStack.ItemNode) -> bool:
	...
	game_item.remove_from_inventory_and_put_in_world.emit(game_item, game_item.global_position, game_item.global_rotation_degrees)
	...
	game_item.state = GameItem.ItemState.PHYSICS_ENABLED
