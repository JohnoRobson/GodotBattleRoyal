...
func _on_game_scene_game_lost() -> void:
	$MenuManager.open_death_menu()

func _on_game_scene_game_won(winner: Team) -> void:
	clear_game_scenes()
	
	$MenuManager.open_win_menu(winner)
	get_tree().paused = true

func _on_game_scene_game_loaded() -> void:
	$MenuManager.clear_menus()
...
func _on_game_hud_pause_button_pressed() -> void:
	toggle_pause_menu()
