class_name ItemInteractionArea extends Area3D
...
func get_items_in_area() -> Array[GameItem]:
	...
	var untyped_array_of_items_in_area = get_overlapping_bodies().filter(func(a): return a is GameItem and a.state == GameItem.ItemState.IN_WORLD)
