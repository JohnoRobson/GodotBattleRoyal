...
func _make_local_inaccuracy_vector() -> Vector3:
	...
	var angle_of_inaccuracy_for_this_shot = random.randf_range(0.0, PI*2)
	# get the inaccuracy vector, which is only up+down inaccuracy, then rotate it
	return Vector3(0, sin(radians_of_inaccuracy_for_this_shot), -cos(radians_of_inaccuracy_for_this_shot)).rotated(Vector3.FORWARD, angle_of_inaccuracy_for_this_shot)
